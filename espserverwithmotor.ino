...
void updateMotorPositions() {
  ...
  currentPosition = positions[1]; // we use motor on port 1 in lab
...
void moveMotor(AsyncWebServerRequest *request) {
  ...
  if (request->hasParam("encoderValue")) {
    ...
    int encoderValue = encoderValueString.toInt();
  }
  else {
    ...
    moveUp(encoderValue);
    Serial.println("moved to :" + "currentPosition");
